...
def start(update, context):
    ...
    if update.message.chat.type == "private":
        sendMessage(f"Hey <b>{update.message.chat.first_name}</b>. Welcome to <b>MHJoyBotsSearch Bot</b>", context.bot, update)
    else:
        sendMessage("I'm ready to Search :)", context.bot, update)
...
def list_drive(update, context):
    ...
    try:
        ...
        msg, button = "Oops.. Something weird happened.. ( • ̀ω•́ )✄╰U╯. Probably my ____ (content) was too big for " \
                      "telegraph's _____ (page).Try Again With The Same Command!", None
        LOGGER.exception(e)
